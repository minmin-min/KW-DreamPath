...
def _fetch_similar_chunks(query_embedding, top_k=1):
    ...
    cur.execute(
        ...
        WHERE dc.category = '강의정보'               -- ✅ 강의정보 카테고리만
...
def recommend_one_course(user_query):
    """
    사용자의 입력 문장을 기반으로 '강의정보' 카테고리 내에서
    가장 유사한 1개 과목(doc_id)을 찾아 모든 청크(단, index≠2)를 출력
    """
    # 1️⃣ 사용자 입력 임베딩 생성
    ...
    # 3️⃣ 가장 유사한 강의의 doc_id 추출
    best_doc_id = rows[0][0]
    best_sim = round(float(rows[0][4]), 4)

    # 4️⃣ 해당 과목의 모든 청크 가져오기 (index=2 제외)
